...
**Status: Phase 5 qualification started; no trained risk model.** Ingestion/review, offset-well comparison and cited full-text search support fixed synthetic replay, evidence-backed alerts, lifecycle actions and engineer feedback. A [scanned public report trial](docs/phase-2/real-source-qualification.md) has processed 58 pages and staged cited incidents for review; its depth and rights gates remain open. Model readiness and a local prediction-window validator are available. [Public dataset research](docs/phase-5/dataset-candidates.md) identifies a Utah FORGE qualification pilot, not a validated training set. No live eRTMAC is connected. Semantic embeddings remain open. See [Phase 2](docs/phase-2/README.md), [Phase 3](docs/phase-3/README.md), [Phase 4](docs/phase-4/README.md) and [Phase 5 data requirements and limits](docs/phase-5/README.md).
...
```text
backend/          API, migrations, ingestion worker, extraction/review tests
database/         PostgreSQL image with spatial/vector extensions
frontend/         editorial evidence review and well directory
